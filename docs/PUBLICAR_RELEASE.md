...
Para a preparação guiada, inclusive por IA local pequena, comece com `python PREPARAR_RELEASE.py --version VERSAO`. Consulte [o guia curto](PREPARAR_RELEASE_IA.md). O comando automatiza preparação e verificações locais; não autoriza nem executa as etapas de commit/push/tag abaixo.
