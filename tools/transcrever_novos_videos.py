...
videos_to_transcribe = [
    "C:/Users/Paulo Henrik/Videos/VID_20260702_164213854.mp4",
    "C:/Users/Paulo Henrik/Videos/VID_20260703_222156268.mp4",
    "C:/Users/Paulo Henrik/Videos/VID_20260703_223034679.mp4"
]
...
for video_path in videos_to_transcribe:
    ...
    if not video_path_obj.exists():
        print(f"Erro: O arquivo {video_path} não existe. Pulando...")
        continue
        
    output_txt = scratch_dir / f"transcricao_{video_path_obj.stem}.txt"
    
    # Se já existir transcrição, não repete
    if output_txt.exists():
        print(f"A transcrição para {video_path_obj.name} já existe em {output_txt}. Pulando...")
        continue
        
    print(f"\n--- Iniciando transcrição de {video_path_obj.name} ---")
    try:
        ...
        with open(output_txt, "w", encoding="utf-8") as f:
            f.write(f"Transcrição do Vídeo: {video_path_obj.name}\n")
        ...
        print(f"Erro ao transcrever {video_path_obj.name}: {e}")
