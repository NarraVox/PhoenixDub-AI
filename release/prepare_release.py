...
def prepare(destination, base, resolved_remote=()):
    ...
    changed = set(names(git('diff', '--name-only', '-z', 'HEAD')))
    ...
    with tempfile.TemporaryDirectory() as temp:
        for name in selected:
            ...
            if name in remote_files:
                ...
                try:
                    ancestor = git('show', f'HEAD:{name}')
                except subprocess.CalledProcessError:
                    ancestor = b''
                if remote != ancestor and data != remote and name not in resolved_remote:
                    paths = [Path(temp)/part for part in ('local', 'base', 'remote')]
                    for path, content in zip(paths, (data, ancestor, remote)):
                        path.write_bytes(content)
                    merged = subprocess.run(['git','merge-file','-p', *map(str, paths)], capture_output=True)
                    if merged.returncode:
                        raise ValueError('Conflito com alterações remotas: ' + name + '. Resolva antes de preparar.')
                    data = merged.stdout
    ...
    report = {'base': base_sha, 'destination': str(destination), 'resolved_remote': list(resolved_remote), 'included_changes': selected,
